...
//We should be able to set the waitingTimes in the library
...
class ADS123X
{
	public:
		...
		ERROR_t read_average(Channel channel, float& avg_value, float* values_raw, byte times = 10, bool Calibrating = false);
		
		// returns (read_average() - OFFSET), that is the current value without the tare weight; times = how many readings to do
		ERROR_t get_value(Channel channel, float& offset_value, float* offset_values_holder, float* values_raw, byte times = 10, bool Calibrating = false);

		// returns get_value() divided by SCALE, that is the raw value divided by a value obtained via calibration
		// times = how many readings to do
		ERROR_t get_units(Channel channel, float& unit_value, float* unit_values_holder, float* values_raw, byte times = 10, bool Calibrating = false);
