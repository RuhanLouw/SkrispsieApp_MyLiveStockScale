...
void ADS1234::begin(byte pin_DOUT, byte pin_SCLK, byte pin_PDWN, byte pin_GAIN0, byte pin_GAIN1, byte pin_SPEED, byte pin_A0, byte pin_A1_or_TEMP, Gain gain, Speed speed){
  _pin_DOUT = pin_DOUT;
  _pin_SCLK = pin_SCLK;
  _pin_PDWN = pin_PDWN;
...
ERROR_t ADS1234::read_average(Channel channel, float& value, byte times, bool Calibrating) {
...
ERROR_t ADS1234::get_value(Channel channel, float& value, byte times, bool Calibrating) {
	float val = 0;
	ERROR_t err;
...
ERROR_t ADS1234::get_units(Channel channel, float& value, byte times, bool Calibrating) {
	float val = 0;
	ERROR_t err;
...
ERROR_t ADS1234::tare(Channel channel, byte times, bool Calibrating) {
	ERROR_t err;
	float sum = 0;
